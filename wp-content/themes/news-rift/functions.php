 ...
 * @package News Rift
 */
if ( ! function_exists( 'news_rift_enqueue_styles' ) ) :
	/**
	 * @since 0.1
	 */
	function news_rift_enqueue_styles() {
		wp_enqueue_style( 'newsup-style-parent', get_template_directory_uri() . '/style.css' );
		wp_enqueue_style( 'news-rift-style', get_stylesheet_directory_uri() . '/style.css', array( 'newsup-style-parent' ), '1.0' );
		wp_enqueue_style('bootstrap', get_template_directory_uri() . '/css/bootstrap.css');
		wp_dequeue_style( 'newsup-default',get_template_directory_uri() .'/css/colors/default.css');
		wp_enqueue_style( 'news-rift-default-css', get_stylesheet_directory_uri()."/css/colors/default.css" );
		if(is_rtl()){
...
add_action( 'wp_enqueue_scripts', 'news_rift_enqueue_styles', 9999 );

function news_rift_theme_setup() {

	//Load text domain for translation-ready
	load_theme_textdomain('news-rift', get_stylesheet_directory() . '/languages');
	...
	'top_right' => __( 'Top Header menu', 'news-rift' ),
	) );
...
add_action( 'after_setup_theme', 'news_rift_theme_setup' );


add_action( 'customize_register', 'news_rift_customizer_rid_values', 1000 );
function news_rift_customizer_rid_values($wp_customize) {
 ...
 */
function news_rift_widgets_init() {
	

	register_sidebar( array(
		'name'          => esc_html__( 'Front-Page Left Sidebar Section', 'news-rift'),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Front-Page Right Sidebar Section', 'news-rift'),
...
add_action( 'widgets_init', 'news_rift_widgets_init' );


function news_rift_remove_some_widgets(){
// Unregister Frontpage sidebar
unregister_sidebar( 'front-page-sidebar' );
}
add_action( 'widgets_init', 'news_rift_remove_some_widgets', 11 );